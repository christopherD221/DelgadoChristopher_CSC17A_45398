...
void filBoat(int [], const int, ComBoard<int> &);                         //Fills computers boats with random values 
void usrBoat(int [], const int, ComBoard<int> &);                         //Fills player choice for boats
void dsplyBd(int[], const int, ComBoard<int> &, UsrBoard &, int[]);       //Displays user and computer board
void hitMiss(ComBoard<int> &, int [], const int, int);                    //Function that figures out if player got a hit or a miss
void cmpChce(UsrBoard &, int [], const int);                         //Function that figures out if computer got a hit or a miss
bool winLose(int [], int[], ComBoard<int> &, bool, bool &);               //Determines whether or not the game has been won or lost
void hiScore(ofstream &, ifstream &, ComBoard<int> &);                    //Opens high score file and checks if you have high score
void statFle(Stats &, ofstream &, ifstream &, ComBoard<int> &);           //Puts Stats structure into a binary file
void fndSize(short &);                                               //User inputs desired size of game board
...
int main(int argc, char** argv) {
    ...
    short size;                              //Later gets passed into class constructor
    ...
    //Create computer and user board objects
    ComBoard<int> cBoard(gmeStat.name, size);
    UsrBoard uBoard(gmeStat.name, size);
    
    //Assigns random value to boats
    filBoat(boats, NBOATS, cBoard);   
    ...
    //Fills user boats
    usrBoat(pBoats, NBOATS, cBoard);
    
    //Displays both boards before any turns have been made
    dsplyBd(pBoats, NBOATS, cBoard, uBoard, boats);
    
    do{
        //Game play function that determines a hit or a miss
        hitMiss(cBoard, boats, NBOATS, choice);
      
        //Computers Choice      
        cmpChce(uBoard, pBoats, NBOATS);
        
        //Displays user and computers altered board
        dsplyBd(pBoats, NBOATS, cBoard, uBoard, boats);
       
        //Function either returns false or true
        loop=winLose(boats, pBoats, cBoard, loop, win);
        
    }while(loop);
    
    //If you win then it will display game stats and high score
    if(win==true){
        statFle(gmeStat, outStat, inptSta, cBoard);
        hiScore(highScr, inptScr, cBoard);
    }
...
void srtScrn(char start, const int NBOATS, Stats &n){
...
void dsplyBd(int boat[], const int NBOATS, ComBoard<int> &g, UsrBoard &u, int boats[]){
...
void filBoat(int a[], const int NBOATS, ComBoard<int> &g){
...
void usrBoat(int a[], const int NBOATS, ComBoard<int> &g){
...
void hitMiss(ComBoard<int> &g, int boats[], const int NBOATS, int choice){
    ...
    g.hitMiss(hit, choice);
}

//Used for the computers turn  
void cmpChce(UsrBoard &g, int pBoats[], const int NBOATS){
    bool hit=false;
    ...
    g.hitMiss(hit, cChoice);
}

//Function that informs if game has been won or lost
bool winLose(int boats[], int pBoats[], ComBoard<int> &g, bool loop, bool &win){
...
void hiScore(ofstream &oFile, ifstream &iFile, ComBoard<int> &g){
...
void statFle(Stats & game, ofstream &oFile, ifstream &iFile, ComBoard<int> &g){
